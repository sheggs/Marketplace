...
public class AuctionHouse {
    ...
    for (Map.Entry<Product, User> pairs : this.forSaleProducts.entrySet()) {
      ...
      if (product == pairs.getKey()) {
        ...
        if (pairs.getKey().isProductSold()) {
          ...
          this.forSaleProducts.remove(product);
          // To stop the for loop from continuing unnecessarily.
          break;
  ...
  /**
   *
   * @return The highest valued sold product.
   */
  public String getHighestValuedSoldProducts() {
    // Initialising a variable to store the highest sold object.
    String highestSold = "";
    // Initialising a variable to store the highest price.
    double highestPrice = Double.MIN_VALUE;
    // Loop through entry sets of the soldProducts hashmap.
    for (Map.Entry<Product, User> pairs : this.soldProducts.entrySet()) {
      // Checks if the price of the product is greater than the current highest price.
      if (pairs.getKey().getCurrentPrice() > highestPrice) {
        // Stores the item into the highest sold variable.
        highestSold = pairs.getKey().getProductId() + " - price of £" + pairs.getKey().getCurrentPrice() + "\n";
        // Updates the highest price with the products price.
        highestPrice = pairs.getKey().getCurrentPrice();
      }
      // Checks if the value of the product is the same as the highest price.
      else if (pairs.getKey().getCurrentPrice() == highestPrice) {
        // Adds product data into the highestSold variable.
        highestSold += pairs.getKey().getProductId() + " - price of £" + pairs.getKey().getCurrentPrice() + "\n";
      }
    }
    return highestSold;
  ...
  public boolean register(Product product, User user) throws IllegalArgumentException {
    ...
    if ((product == null) || (user == null)) {
      // Throws a custom error message since the parameters aren't properly initialised.
      throw new IllegalArgumentException("Your product or user is not properly initialised");
    }
    else {
